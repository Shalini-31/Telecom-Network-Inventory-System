...
public class DeleteOperator extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException
	{
		...
		RequestDispatcher rd=request.getRequestDispatcher("viewOperator");
		try
		{
			Connection connection=Databaseconnection.initializeDataBase();
			PreparedStatement preparedStatement=connection.prepareStatement("delete from operator where username=?;");
			preparedStatement.setString(1,usernameToDelete);
			int result=preparedStatement.executeUpdate();
			if(result==1)
			{
				out.println("<script>alert('Operator deleted');</script>");
